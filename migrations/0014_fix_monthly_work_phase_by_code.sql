...
-- Korjaa Phase 18 kuukausiraportin yhdistämään litterat KOODILLA, ei UUID:lla
...
SELECT
  ...
  CAST(ROUND(SUM(COALESCE(r.budget_amount, 0)), 2) AS numeric(14,2)) AS target_total,
  CAST(ROUND(SUM(COALESCE(r.actual_amount, 0)), 2) AS numeric(14,2)) AS actual_total,
  CAST(ROUND(SUM(COALESCE(r.forecast_value, 0)), 2) AS numeric(14,2)) AS forecast_total
...
JOIN litteras l
  ON l.littera_id = m.littera_id
JOIN v_target_month_cost_report r
  ON r.project_id = wp.project_id
 AND r.target_littera_id = l.littera_id
GROUP BY
  wp.project_id,
  lb.work_phase_id,
  wp.name,
