...
import type { DemoQuickLogin } from "../../server/demoQuickLogins";
...
export default function LoginForm({
  ...
  const quickLoginGroups = demoMode
    ? [...new Set(quickLogins.map((entry) => entry.group))].map((group) => ({
        group,
        users: quickLogins.filter((entry) => entry.group === group)
      }))
    : [];

  return (
    <>
      <div className={demoMode && quickLogins.length > 0 ? "login-split" : undefined}>
        {demoMode && quickLogins.length > 0 ? (
          <section className="quick-login-panel" aria-label="Pikakirjautuminen">
            <div className="quick-login-header">
              <h2>Pikakirjautuminen</h2>
              <p className="muted">Demo-roolit yhden klikkauksen kirjautumiseen.</p>
            </div>
            <div className="quick-login-groups">
              {quickLoginGroups.map((group) => (
                <div className="quick-login-group" key={group.group}>
                  <div className="quick-login-group-title">{group.group}</div>
                  <div className="quick-login-grid">
                    {group.users.map((entry) => (
                      <form key={entry.username} action={formAction}>
                        <input type="hidden" name="username" value={entry.username} />
                        <input type="hidden" name="quick" value="1" />
                        <button className="btn btn-secondary btn-sm quick-login-btn" type="submit">
                          {entry.label}
                        </button>
                      </form>
                    ))}
