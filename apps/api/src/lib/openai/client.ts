...
import AgentsOpenAI from "@openai/agents-openai/node_modules/openai";
...
export function initOpenAI(): void {
  if (!agentsConfigured) {
    const agentsClient = new AgentsOpenAI({ apiKey: getOpenAIKey() });
    setDefaultOpenAIClient(agentsClient);
